// fbased on  https://github.com/foize/go.fifo/blob/master/fifo.go
...
type chunk struct {
	items       [chunkSize]Task        // list of queue'ed items
...
func (q *Queue) Add(item Task) {
	// locking to make Queue thread-safe
	q.lock.Lock()
	defer q.lock.Unlock()

	// check if item is valid
	if reflect.TypeOf(item) != reflect.TypeOf(Task{}) {
		panic("can not add other than Task type object item to fifo queue")
	}
